...
public class World extends JFrame {
    ...
    public JPanel addBotStats(Bot b, int x, int y) {
        ...
        //TODO YOU need to Buy The NetworkGUI bundle, delete the JTextArea and uncomment the NetworkPanel
        
        JTextArea message = new JTextArea("You need to add the \"NetworkGUI\" bundle\n"
        		+ "row 339 in World.java for more details");
        message.setBounds(20, 60 * 4 + 20, 300, 200);
        ret.add(message);
        
       /*  you need to uncomment this section and make sure that Networkpanel is availble           NetworkPanel con = new NetworkPanel(b.agent.getNet());
    ...
    public void paint(Graphics g) {
        ...
        if (mapLoaded && population.size() < MIN_POP_SIZE) {
           ...
           /* int neu = (int) (Math.random() * 4);
            ...
            if (Bot.useMemorie) {
                b = new Bot(new Agent(new Network(14, 4, 2, new int[]{14, 8})), this, 0);
            } else {
                b = new Bot(new Agent(new Network(5, 4, 1, new int[]{(int) (Math.random() * 8 + 4)})), this, 0);
            }
 ...
 */
            ...
            for (int j = 0; j < botCount; j++) {
                ...
                //TODO in data bleiben 3 Ã¼berflÃ¼ssige werte Ã¼brig kp was lÃ¤uft

                subData = data.substring(0, data.indexOf('}') + 1);
                b.agent = new Agent(new Network(subData));
