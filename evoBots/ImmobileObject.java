...
public class ImmobileObject extends JComponent {
    ...
    public Component getObjectAt(int x, int y) {
        ...
        if (this.getParent() != null) {
            // Kontrolliere ob neue Position auÃ¯Â¿Â½erhalb des Frames liegt
            if (x < 0 || y < 0 || x + this.getWidth() > this.getParent().getWidth() ||
                    y + this.getHeight() > this.getParent().getHeight())
                // In diesem Fall wird der contentPane des Formulars Ã¯Â¿Â½bergeben
                ret = this.getParent();
            else {
                // Kontrolliere ob sich die neue Position mit anderen Objekten Ã¯Â¿Â½berdeckt
                ...
                while (komponenten != null && i < komponenten.length && ret == null) {
                    ...
                    // mit dem zu Kontrollierendem zusammenfÃ¯Â¿Â½llt
